...
class NewsController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'contents.*.video_height'  => 'nullable|integer|min:10|max:350',
        ...
        DB::transaction(function () use ($request, $validated) {
            ...
            foreach ($validated['contents'] as $index => $item) {
                ...
                if ($item['type'] === 'video') {
                    ...
                    $data['video_height']       = $item['video_height'] ?? 350;
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'contents.*.video_orientation' => 'nullable|in:landscape,portrait',
            'contents.*.video_width' => 'nullable|integer|min:30|max:100',
            'contents.*.video_height' => 'nullable|integer|min:10|max:800',
            'contents.*.video_align' => 'nullable|in:left,center,right',
            'contents.*.video_radius' => 'nullable|integer|min:0|max:40',
        ...
        DB::transaction(function () use ($request, $validated, $news) {
            ...
            // 1. Hapus konten yang di-remove dari UI
            $deleteIds = array_diff($existingIds, $requestIds);
            foreach ($deleteIds as $deleteId) {
                $content = $news->contents->firstWhere('id', $deleteId);
                if ($content) {
            ...
            // 2. Loop & Update / Create Konten
            foreach ($validated['contents'] as $index => $item) {
                $existingContent = !empty($item['id']) ? $news->contents->firstWhere('id', $item['id']) : null;
                ...
                if ($item['type'] === 'image') {
                    if (isset($item['image'])) {
                        // Hapus gambar lama jika ada file baru di-upload
                        if ($existingContent && $existingContent->image_path && file_exists(public_path('images/articles/' . $existingContent->image_path))) {
                            unlink(public_path('images/articles/' . $existingContent->image_path));
                        }
                ...
                if ($item['type'] === 'video') {
                    if (isset($item['video'])) {
                        // Hapus video lama jika ada file baru di-upload
                        if ($existingContent && $existingContent->video_path && file_exists(public_path('videos/articles/' . $existingContent->video_path))) {
                            unlink(public_path('videos/articles/' . $existingContent->video_path));
                        }
                    ...
                    // Tambahkan setting video
                    $data['video_orientation'] = $item['video_orientation'] ?? 'landscape';
                    $data['video_width'] = $item['video_width'] ?? 100;
                    $data['video_height'] = $item['video_height'] ?? 800;
                    $data['video_align'] = $item['video_align'] ?? 'center';
                    $data['video_radius'] = $item['video_radius'] ?? 12;
                    $data['caption'] = $item['caption'] ?? null;
                }
